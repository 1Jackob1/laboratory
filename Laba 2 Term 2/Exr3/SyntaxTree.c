...
t_node* New_Node_Oper(char op, t_node *lhs, t_node *rhs) {
    t_node *node = New_Node();
    if (node == NULL)
        return NULL;
    node->type = IS_OPR;
    node->t_data.opr = op;
...
t_node* Diff_Tree_Node(t_node *node) {
    if (node == NULL)
        return NULL;
    t_node *tmp;
    switch (node->type) {
        case IS_NUM: {
            if ((tmp = New_Node()) == NULL) return NULL;
            tmp->type = IS_NUM;
            tmp->t_data.num = 0;
            return tmp;
        }
        ...
        case IS_OPR:
            switch (node->t_data.opr) {
                case '$':
                    return New_Node_Oper('$', Diff_Tree_Node(node->CHILD[0]), NULL);
                case '-':
                case '+':
                    return New_Node_Oper(node->t_data.opr, Diff_Tree_Node(node->CHILD[0]),
                                                           Diff_Tree_Node(node->CHILD[1])
                    );
                case '*':
                    return New_Node_Oper('+',
                           New_Node_Oper('*', Copy_Tree_Nodes(node->CHILD[0]),
                                              Diff_Tree_Node(node->CHILD[1])),
                           New_Node_Oper('*', Diff_Tree_Node(node->CHILD[0]),
                                              Copy_Tree_Nodes(node->CHILD[1])
                   ));
                case '/':
                    return New_Node_Oper('/', New_Node_Oper('-', New_Node_Oper('*', Diff_Tree_Node(node->CHILD[0]), Copy_Tree_Nodes(node->CHILD[1])),
                                                                 New_Node_Oper('*', Diff_Tree_Node(node->CHILD[1]), Copy_Tree_Nodes(node->CHILD[0]))),
                                                                 New_Node_Oper('*', Copy_Tree_Nodes(node->CHILD[1]), Copy_Tree_Nodes(node->CHILD[1]))
                    );
            }
            case IS_FUN:{
                //{ [0]&Sin, [1]&Cos, [2]&Exp, [3]&Sqrt, [4]&Log }
                switch (node->t_data.ind){
                   case 0: {
                       if (tmp == NULL)
                        return NULL;

                       tmp = Copy_Tree_Nodes(node);
                       tmp->t_data.ind = 1;

                       return New_Node_Oper('*', tmp, Diff_Tree_Node(node->CHILD[0]));
                   }
                   case 1: {
                       if (tmp == NULL)
                        return NULL;
                       tmp = Copy_Tree_Nodes(node);
                       tmp->t_data.ind = 0; //(cos)` -> -sin

                       return New_Node_Oper('$', New_Node_Oper('*', tmp, Diff_Tree_Node(node->CHILD[0])), NULL);
                   }
                   case 2: {
                       return New_Node_Oper('*', Copy_Tree_Nodes(node), Diff_Tree_Node(node->CHILD[0]));
                   }

                   case 3: {
                        t_node *tmp4 = New_Node();
                        t_node *tmp5 = New_Node();
                        if(tmp4 == NULL || tmp5 == NULL)
                            return NULL;
                        tmp4->type = IS_NUM;
                        tmp4->t_data.num = 1;
                        ...
                        return New_Node_Oper('*',
                               New_Node_Oper('/', tmp4,
                               New_Node_Oper('*', tmp5, Copy_Tree_Nodes(node->CHILD[0]))),
                                                        Diff_Tree_Node(node->CHILD[0]));


                   }
                   case 4: {
                       return New_Node_Oper('/', Diff_Tree_Node(node->CHILD[0]), Copy_Tree_Nodes(node->CHILD[0]));
                   }
                }
            }
    }
...
t_tree* Diff_Tree(t_tree *src) {
    t_tree *new_tree = malloc(sizeof(t_tree));
    if (new_tree == NULL)
        return NULL;
    new_tree->root = Diff_Tree_Node(src->root);
}
